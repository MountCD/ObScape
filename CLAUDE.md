...
cargo run -p ob_core -- --help   # CLI flags (--config, --database, --init, --verbose, --allow-autogenerated)
cargo test                       # unit tests (ob_common) + handler tests (ob_core/src/server.rs)
cargo test -p ob_common <name>   # run a single test by name in one crate
OBSCAPE_TEST_DATABASE_URL=postgres://... cargo test -p ob_core   # also runs the end-to-end chat test against a real Postgres
```
...
The template written by `--init` sets `autogenerated = true`; `load_config` refuses to start while that flag is set (opens the file via `xdg-open`/`notepad` and returns `ConfigError::Autogenerated`), forcing the user to edit and clear it. `--allow-autogenerated` explicitly bypasses this check; `--verbose` no longer does.
